...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: Partial<HyperspeedOptions> }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      initPasses() {
        ...
        const smaaEffect = new (SMAAEffect as any)({
      ...
      update(delta: number) {
        ...
        if ((this.options.distortion as any).getJS) {
          const distortion = (this.options.distortion as any).getJS(0.025, time);
      ...
      init() {
        ...
        let material = new THREE.ShaderMaterial({
          ...
          uniforms: Object.assign(
            ...
            options.distortion && (options.distortion as any).uniforms ? (options.distortion as any).uniforms : {}
          )
        });

        material.onBeforeCompile = shader => {
          if (options.distortion && (options.distortion as any).getDistortion) {
            shader.vertexShader = shader.vertexShader.replace(
              '#include <getDistortion_vertex>',
              (options.distortion as any).getDistortion
            );
          }
        };
    ...
    class LightsSticks {
      ...
      init() {
        ...
        let colors: any = options.colors.sticks;
        if (Array.isArray(colors)) {
          colors = colors.map(c => new THREE.Color(c));
        } else {
          colors = new THREE.Color(colors);
    ...
    class Road {
      ...
      createPlane(side: number, width: number, isRoad: boolean) {
        ...
        const material = new THREE.ShaderMaterial({
          ...
          side: THREE.DoubleSide,
          uniforms: Object.assign(uniforms, this.webgl.fogUniforms, options.distortion && (options.distortion as any).uniforms ? (options.distortion as any).uniforms : {})
        });
    ...
    (options as any).distortion = distortions[options.distortion];
